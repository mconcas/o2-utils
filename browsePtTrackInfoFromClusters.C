...
void browsePtTrackInfoFromClusters(const std::string path = "./",
      ...
      if (0 == geom->getLayer(c.getSensorID())) {
        ...
        if (lbl.isValid() && !lbl.isFake() && lbl.getSourceID() == 0) {
          ...
          if (!(lbl.getTrackID() == 2147483647)) {
            ...
            std::cout << "\tid: " << lbl.getTrackID() << " PDG: " << mcTr.GetPdgCode() << " pT: " << mcTr.GetPt() << "\n";
          } else {
            std::cout << "\t -> " << lbl.getTrackID() << " evt ID: " << lbl.getEventID() << std::endl;
          }
        }
      }
    }
